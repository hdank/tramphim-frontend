...
const MovieImages = ({ imagesData }) => {
  ...
  const backdrops = images
    .filter((image) => image.loai_anh === "backdrop")
    .slice(0, 5);

  const posters = images
    .filter((image) => image.loai_anh === "poster")
    .slice(0, 5);
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-5 items-stretch gap-1.5 lg:grid-cols-8">
        {finalImages.map((image) => (
          <div
            key={image.id}
            className={`overflow-hidden ${
              image.loai_anh === "backdrop" ? "col-span-2" : "col-span-1"
            }`}
          >
            <img
