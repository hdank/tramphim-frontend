...
const categoryOptions = [
  ...
  const fetchMovies = useCallback(
    async (pageToFetch = 1) => {
      ...
      try {
        ...
        setLoading(false);
        // ✨ ĐÃ BỎ: setIsFilterOpen(false); để bộ lọc không tự động đóng
      }
    },
    [buildApiUrl, filters],
  );
  ...
  // Hàm mới: Chuyển đổi trạng thái mở/đóng của bộ lọc
  const toggleFilter = () => {
    setIsFilterOpen(prev => !prev);
  };
  
  // ... (giữ nguyên handleInputChange, handleInputKeyDown, FilterGroup)
      ...
      <div className="hide-scrollbar flex flex-row items-center gap-1 overflow-x-auto md:flex-wrap md:overflow-visible lg:gap-4">
        {options.map((option) => (
          <button
            ...
            className={`whitespace-nowrap rounded-[4px] px-3 py-1.5 transition-colors duration-200 ${
  ...
  return (
    <div className="min-h-screen">
      ...
      <div 
        id="filter-panel"
        // Thay đổi ở đây: max-h-screen để tạo hiệu ứng trượt xuống
        className={`flex flex-col gap-4 transition-all duration-300 ease-in-out ${
          isFilterOpen ? "max-h-screen opacity-100" : "max-h-0 opacity-0 overflow-hidden"
        }`}
      >
      ...
      <div className="py-4 flex flex-row items-center justify-between">
        <div className="">
          <div className="flex items-center gap-3">
            ...
            <h1 className="bg-gradient-to-r from-white via-sky-100 to-cyan-100 bg-clip-text text-lg font-bold text-transparent lg:text-2xl">
              Phim {tittle}
            </h1>
          </div>
      ...
      {!loading && !error && movies.length === 0 && (
        <div className="py-20 flex flex-col items-center justify-center space-y-4 bg-[#242424] rounded-xl shadow-inner border border-gray-700/50 mt-8">
         
          <h2 className="text-base lg:text-2xl font-extrabold text-white">
            Không Tìm Thấy Phim Nào Phù Hợp
          </h2>
          <p className="text-sm lg:text-base text-gray-400 max-w-lg text-center">
            Hãy Dùng Bộ Lọc Để Tìm Kiếm Phim
          </p>
