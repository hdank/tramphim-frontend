...
export default function CategoryMovies({
  ...
  const fetchMovies = useCallback(
    async (pageToFetch = 1) => {
      ...
      try {
        ...
        // Thêm các filters hiện tại (chỉ những cái có giá trị)
        Object.entries(filters).forEach(([key, value]) => {
          if (value) {
        ...
        // Xóa tham số page nếu là trang 1
        if (pageToFetch > 1) {
  ...
  const handleApplyFilters = () => {
    fetchMovies(1);
    // Tùy chọn: Đóng bộ lọc sau khi áp dụng
    // setIsFilterOpen(false);
  };

  // Thêm hàm reset tất cả bộ lọc về mặc định
  const handleClearFilters = () => {
    // Sử dụng lại logic khởi tạo để reset về mặc định của trang hiện tại
    setFilters(getInitialFilters(null, initialSlug)); 
    fetchMovies(1); // Fetch lại với bộ lọc mặc định/rỗng
  };
        ...
        {options.map((option) => (
          <button
            ...
            className={`whitespace-nowrap rounded-[4px] px-3 py-1.5 transition-colors duration-200 ${filters[filterKey] === option.value
                ...
                initialSlug === "phim-chieu-rap" &&
                option.value !== ""
                ? "cursor-not-allowed opacity-70"
                : ""
              }`}
            // ⭐️ ĐÃ SỬA: Giữ lại logic vô hiệu hóa ban đầu của bạn cho nhóm Chiếu Rạp (nếu muốn)
            // nhưng đã loại bỏ logic vô hiệu hóa cho nhóm Loại Phim.
            disabled={
              filterKey === "chieu_rap" &&
              initialSlug === "phim-chieu-rap" &&
              option.value !== ""
    ...
    <div className="min-h-screen">
      <div className="mb-4 flex justify-start ">
        ...
        >
          ...
          {/* Thêm biểu tượng mũi tên để chỉ trạng thái */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
      ...
      {/* 4. Thêm container có thể ẩn/hiện cho các FilterGroup */}
      <div
        id="filter-panel"
